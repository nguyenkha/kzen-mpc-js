...
function encrypt(key, data) {
  ...
  const cipher = createCipheriv(CYPHER, Buffer.from(key, 'hex'), iv);
  let encrypted = cipher.update(Buffer.from(data, 'hex'));
...
function decrypt(key, data) {
  const decipher = createDecipheriv(CYPHER, Buffer.from(key, 'hex'), Buffer.from(data.iv, 'hex'));
  decipher.setAuthTag(Buffer.from(data.tag, 'hex'));
  const decrypted = decipher.update(Buffer.from(data.encrypted, 'hex'));
  return Buffer.concat([decrypted, decipher.final()]).toString('hex');
}
    ...
    if (!this.results) {
      ...
      switch (this.step) {
        case 0: {
          ...
          }).map(k => k.padStart(64, 0));
          result = {
            vssScheme,
            secretShare: secretShare.map((s, i) => encrypt(this.encryptionKeys[i], s.padStart(64, 0))),
          };
          break;
        }
        case 1: {
          ...
          this.partyShares = this.results
            ...
            .map((e, i) => decrypt(this.encryptionKeys[i], e));
    ...
    if (!this.results) {
      ...
      switch (this.step) {
        case 0: {
          ...
          result = {
            vssScheme,
            secretShare: secretShare.map((s, i) => encrypt(this.share.encryptionKeys[this.share.parties.indexOf(this.parties[i])], s.padStart(64, 0))),
          };
          break;
        }
        case 1: {
          ...
          this.partyShares = this.results
            .map(r => r.secretShare[this.parties.indexOf(this.index)])
            .map((e, i) => decrypt(this.share.encryptionKeys[this.share.parties.indexOf(this.parties[i])], e));
    ...
    if (!this.results) {
      ...
      switch (this.step) {
        case 0: {
          ...
          this.stage2Result = Ok;
          result = {
            mbgi: this.parties.map((p, i) => p === this.index ? null : encrypt(this.share.encryptionKeys[this.share.parties.indexOf(p)], Buffer.from(JSON.stringify(this.stage2Result.gamma_i_vec[i > this.parties.indexOf(this.index) ? i - 1 : i][0])).toString('hex'))),
            mbwi: this.parties.map((p, i) => p === this.index ? null : encrypt(this.share.encryptionKeys[this.share.parties.indexOf(p)], Buffer.from(JSON.stringify(this.stage2Result.w_i_vec[i > this.parties.indexOf(this.index) ? i - 1 : i][0])).toString('hex'))),
          };
          break;
        }
        case 1: {
          this.mbgis = this.results.map((r, i) => r.mbgi[this.parties.indexOf(this.index)] ? JSON.parse(Buffer.from(decrypt(this.share.encryptionKeys[this.share.parties.indexOf(this.parties[i])], r.mbgi[this.parties.indexOf(this.index)]), 'hex').toString()) : null).filter(m => m);
          const mbwis = this.results.map((r, i) => r.mbwi[this.parties.indexOf(this.index)] ? JSON.parse(Buffer.from(decrypt(this.share.encryptionKeys[this.share.parties.indexOf(this.parties[i])], r.mbwi[this.parties.indexOf(this.index)]), 'hex').toString()) : null).filter(m => m);
