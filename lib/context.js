    ...
    if (!this.results) {
      ...
      switch (this.step) {
        ...
        case 1: {
          ...
          this.partyShares = this.results.map(r => r.secretShare[this.parties.indexOf(this.index)]);
    ...
    if (!this.results) {
      ...
      switch (this.step) {
        ...
        case 1: {
          this.vssSchemes = this.results.map(r => r.vssScheme);
          // TODO: Decrypte secret share
          const partyShares = this.results.map(r => r.secretShare[this.parties.indexOf(this.index)]);
    ...
    if (!this.results) {
      ...
      switch (this.step) {
        case 0: {
          ...
          this.stage2Result = Ok;
          result = {
            mbgi: this.parties.map((p, i) => p === this.index ? null : this.stage2Result.gamma_i_vec[i > this.parties.indexOf(this.index) ? i -  1: i][0]),
            mbwi: this.parties.map((p, i) => p === this.index ? null : this.stage2Result.w_i_vec[i > this.parties.indexOf(this.index) ? i - 1 : i][0]),
          };
          break;
        }
        case 1: {
          this.mbgis = this.results.map(r => r.mbgi[this.parties.indexOf(this.index)]).filter(m => m);
          const mbwis = this.results.map(r => r.mbwi[this.parties.indexOf(this.index)]).filter(m => m);
        ...
        case 3: {
          ...
          const { Err, Ok } = bindings.secp256k1_sign_stage6({
            ...
            message_bn: bindings.hash_bytes_to_big_int(this.message),
          });
          if (Err) {
            throw Error(Err);
          }
